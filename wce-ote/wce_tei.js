...
function getHtmlByTei(inputString, args) {
    ...
    var Tei2Html_ab = function ($htmlParent, $teiNode) {
        ...
        } else if ($teiNode.getAttribute("type") && $teiNode.getAttribute("type") == 'verse') { // Verse
            var $newNode = $newDoc.createElement('span');
        ...
        } else if ($teiNode.getAttribute("type") && $teiNode.getAttribute("type") == 'ritualdirection') { // ritualdirection
            var $newNode = $newDoc.createElement('span');
            var type = $teiNode.getAttribute('type');
...
function getTeiByHtml(inputString, args) {
        ...
        if (wceAttrValue != null && wceAttrValue.match(/verse_number/)) {
            ...
            if (textNode) {
                ...
                if (lang_index > -1) {
                    ...
                    if (pos > -1) {
                        g_verseNode.setAttribute('xml:lang', wceAttrValue.substring(lang_index + 5, lang_index + 5 + pos));
                    } else {
                        g_verseNode.setAttribute('xml:lang', wceAttrValue.substring(lang_index + 5));
                    }
                }
                g_verseNode.setAttribute('n', g_bookNumber + '.' + g_chapterNumber + '.' + g_stanzaNumber + '.' + g_verseNumber);
                /*
                var count_verse = 97 + Math.floor(g_verseNumber/2);

                g_verseNode.setAttribute('xml:id', g_bookNumber + g_chapterNumber + "." + g_stanzaNumber + String.fromCharCode(count_verse) + '-' + g_verseNumber%2)
                g_verseNode.setAttribute('n', String.fromCharCode(count_verse));
                ...
                */
                ...
                /*
                if (wceAttrValue)
                	lang_index = wceAttrValue.indexOf('language_name=');
                if (lang_index > -1) {
                	var amp_index = wceAttrValue.substring(lang_index + 14).indexOf('&');
                	if (amp_index > -1) {
                		if (wceAttrValue.substring(lang_index + 14, lang_index + 14 + amp_index) == 'other') {
                			lang_index = wceAttrValue.indexOf('language_name_other=');
                			if (lang_index > -1) {
                				var amp_index = wceAttrValue.substring(lang_index + 20).indexOf('&');
                				if (amp_index > -1)
                					g_verseNode.setAttribute('xml:lang', decodeURI(wceAttrValue.substring(lang_index + 20, lang_index + 20 + amp_index)));
                			}
                		} else {
                			g_verseNode.setAttribute('xml:lang', wceAttrValue.substring(lang_index + 14, lang_index + 14 + amp_index));
                		}
                	}
                }*/
                if (g_stanzaNode)
            ...
            if (textNode) {
                ...
                if (lang_index > -1) {
                    pos = wceAttrValue.substring(lang_index).indexOf("&");
                    if (pos > -1) {
                        g_stanzaNode.setAttribute('xml:lang', wceAttrValue.substring(lang_index + 5, lang_index + 5 + pos));
                    } else {
                        g_stanzaNode.setAttribute('xml:lang', wceAttrValue.substring(lang_index + 5));
                    }
                }
                if (partial_index > -1) { // node contains information about partial
            ...
            if (textNode) {
                ...
                if (lang_index > -1) {
                    pos = wceAttrValue.substring(lang_index).indexOf("&");
                    if (pos > -1) {
                        g_chapterNode.setAttribute('xml:lang', wceAttrValue.substring(lang_index + 5, lang_index + 5 + pos));
                    } else {
                        g_chapterNode.setAttribute('xml:lang', wceAttrValue.substring(lang_index + 5));
                    }
                }
                g_chapterNode.setAttribute('n', g_chapterNumber);
                //}
                if (partial_index > -1) { // node contains information about partial
                    g_chapterNode.setAttribute('part', wceAttrValue.substring(partial_index + 8, partial_index + 9));
                }
                if (g_bookNode)
            ...
            if (textNode) {
                ...
                if (lang_index > -1) {
                    pos = wceAttrValue.substring(lang_index).indexOf("&");
                    if (pos > -1) {
                        g_bookNode.setAttribute('xml:lang', wceAttrValue.substring(lang_index + 5, lang_index + 5 + pos));
                    } else {
                        g_bookNode.setAttribute('xml:lang', wceAttrValue.substring(lang_index + 5));
                    }
                }
                if (partial_index > -1) { // node contains information about partial
            ...
            if (textNode) {
                ...
                if (lang_index > -1) {
                    pos = wceAttrValue.substring(lang_index).indexOf("&");
                    if (pos > -1) {
                        g_verselineNode.setAttribute('xml:lang', wceAttrValue.substring(lang_index + 5, lang_index + 5 + pos));
                    } else {
                        g_verselineNode.setAttribute('xml:lang', wceAttrValue.substring(lang_index + 5));
                    }
                }
                if (g_stanzaNode) {
                    g_lineNode.setAttribute('n', g_bookNumber + '.' + g_chapterNumber + '.' + g_stanzaNumber + '.' + g_verselineNumber);
                    g_stanzaNode.appendChild(g_verselineNode);
                } else if (g_verseNode) {
            ...
            if (textNode) {
                ...
                if (lang_index > -1) {
                    pos = wceAttrValue.substring(lang_index).indexOf("&");
                    if (pos > -1) {
                        g_lineNode.setAttribute('xml:lang', wceAttrValue.substring(lang_index + 5, lang_index + 5 + pos));
                    } else {
                        g_lineNode.setAttribute('xml:lang', wceAttrValue.substring(lang_index + 5));
                    }
                }
                if (g_stanzaNode) {
                    ...
                    g_lineNode.setAttribute('n', g_bookNumber + '.' + g_chapterNumber + '.' + g_verseNumber + '.' + g_lineNumber);
                    g_verseNode.appendChild(g_lineNode);
                } else
            ...
            if (textNode) {
                ...
                if (lang_index > -1) {
                    pos = wceAttrValue.substring(lang_index).indexOf("&");
                    if (pos > -1) {
                        g_ritualdirectionNode.setAttribute('xml:lang', wceAttrValue.substring(lang_index + 5, lang_index + 5 + pos));
                    } else {
                        g_ritualdirectionNode.setAttribute('xml:lang', wceAttrValue.substring(lang_index + 5));
                    }
                }
                if (g_stanzaNode) {
    ...
    var html2Tei_langchange = function (arr, $teiParent, $htmlNode) {
        ...
        // This could be arr[language_name] as well
        var lang = $htmlNode.getAttribute("language");
